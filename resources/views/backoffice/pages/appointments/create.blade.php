                ...
                <div class="col-md-6">
                    ...
                    <div class="box">
                        <div class="box-header with-border">
                            <h4 class="box-title">Appointment Request Form</h4>
                        </div>
                        <!-- /.box-header -->
                        @include('backoffice.pages.appointments.form')
                    </div>
                </div>
            </div>
        </section>
...
$(function() {
    ...
    $(".input-service").on("change", function(){
        ...
        $.ajax({
            type: "POST",
            url: "{{route('backoffice.services.serviceTypes')}}",
            ...
            success: function(data){
                ...
                $.each(data.datas, function (i, type) {
                    $('.input-service-type').append($('<option>', { 
                        value: type.id,
                        text : type.type 
                    }));
                });
    ...
    $(".input-service-type").on("change", function(){
        let id = $(this).val();

        $.ajax({
            type: "POST",
            url: "{{route('backoffice.services.serviceTypeDetail')}}",
            data: { _id : id , _token : "{{csrf_token()}}" },
            dataType: "json",
            async: true,
            success: function(data){
